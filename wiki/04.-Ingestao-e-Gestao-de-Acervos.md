...
## 🗂️ Biblioteca em Árvore, Smart Bins e Palavras-chave

A barra lateral esquerda organiza o acervo em uma estrutura hierárquica e semântica de alta produtividade:

- **Pastas Reais:** Refletem a estrutura de diretórios do disco (ex: `Dia 01 / Câmera A / Cartão 01`).
- **Smart Bins & Agrupamento por Palavras-chave:** Classificação automática por tags e atributos narrativos gerados pela IA ou atribuídos pelo usuário, com ordenação avançada e suporte a popout multi-tela.
- **Destaque Cromático por Onda Contínua (6px):** Navegação assistida por uma faixa cromática de 6px renderizada com a cor real da mídia e piso calibrado de luminosidade para leitura visual instantânea de blocos.
...
- **Três Modos de Visualização:**
  - 🖼️ **Galeria Clean (Google Fotos):** Layout justificado de ponta a ponta que preserva a proporção nativa dos arquivos (16:9, 4:3, 9:16 vertical), sem faixas cinzas ociosas. No zoom máximo, atua como preview cinematográfico de 100% de largura (`object-fit: contain`).
  - 🎴 **Grade de Cards:** Visual clássico NLE em cards uniformes com pôster e metadados na base.
  - 📜 **Árvore / Lista Detalhada:** Exibição hierárquica por diretórios/subpastas com metadados técnicos estruturados (Resolução, FPS, Duração, Codec, Canais).
- **Subclipes Virtuais Não-Destrutivos (`Ctrl + U`):**
  - Exibidos na biblioteca com badge dedicado `SUB`, miniaturas HQ do ponto de entrada e hover preview delimitado estritamente ao intervalo marcado.
- **Scrubbing Rápido com `Ctrl + Hover`:** Mova o cursor segurando `Ctrl` sobre qualquer clipe para decupagem instantânea com linha ciano e timecode.
- **Pipeline de Miniaturas em Dois Estágios (LQ e HQ):** Resposta imediata com miniaturas leves de 160px durante scrubbing e upgrade silencioso para alta definição (640px) após 180ms de repouso no frame.
- **Momentos Cruciais (Key Moments / Stars):** Marque instantes narrativos com `Ctrl+Shift+Clique` ou pelo botão estrela no Source Player. Momentos marcados ganham pré-geração em background, ticks na régua e slideshow cíclico com a tecla `Alt`.
- **HUD Flutuante Desacoplado (`Shift + Hover`):** Passe o mouse com `Shift` para abrir um card flutuante com título executivo, decupagem IA, falantes, tags e dados técnicos sem alterar o layout da grade.
- **Rotação Ágil Não-Destrutiva (`R` / `Shift + R`):** Alterne a orientação de fotos e vídeos em 0°, 90°, 180° e 270° pelas teclas **`R`** (horário) e **`Shift + R`** (anti-horário) ou pelo menu de contexto, propagando para a timeline e exportação com conformação proporcional perfeita.
- **Zoom Dinâmico de Cards (`Shift + Wheel`):** Segure `Shift` e gire a roda do mouse sobre a galeria para dimensionar os itens de forma contínua até 300px.
- **Sincronização em Janela Destacada:** Se a biblioteca estiver desacoplada em monitor secundário, todas as ações de zoom, duplo clique, modo de visualização, momentos cruciais e atualização de pôsteres permanecem perfeitamente integradas.
...
| :--- | :--- | :--- | :--- |
| **`2x Clique`** | **No Final da Timeline (Append)** | Anexa a mídia no final da timeline (prefere Pista 1 em empate; nunca envia para pista oculta). | Final do clipe inserido |
| **`Shift + 2x Clique`** | **Na Posição da Agulha (Playhead)** | Insere o clipe na agulha da pista ativa/padrão. | Final do clipe inserido |
...
| **`Ctrl + Shift + R`** *(ou `Alt + Arraste`)* | **Substituir Clipe Selecionado (Replace Edit)** | Substitui a mídia do clipe atualmente selecionado na timeline mantendo rigorosamente duração, velocidade, cor e efeitos. | Mantém posição |
