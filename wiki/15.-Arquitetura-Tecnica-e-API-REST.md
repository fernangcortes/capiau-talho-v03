...
| `GET` | `/api/video/{video_id}/thumbnail-at` | Retorna miniatura em segundo específico com suporte a `quality=low` (160px) e `quality=hq` (640px). |
| `POST` | `/api/video/{video_id}/crucial-moments` | Adiciona, remove ou define lista de instantes cruciais (JSON timestamps). |
| `POST` | `/api/video/{video_id}/hover-duration` | Define duração de hover play individual por clipe (NULL = usa global). |
| `POST` | `/api/media/{media_type}/{media_id}/rotate` | Rotaciona mídia em passos de 90° (0°, 90°, 180°, 270°). |
| `POST` | `/api/video/{video_id}/thumbnail` | Define e grava o timestamp da miniatura de capa oficial do vídeo (`thumbnail_time`). |
