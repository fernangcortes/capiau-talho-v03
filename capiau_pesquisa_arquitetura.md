...
**Recomendação CapIAu:** **WhisperX** para ingest batch (transcrição + diarização + timestamps palavra-a-palavra). **faster-whisper** com chunked streaming para modo TV (teleprompter ao vivo), com fallback para AssemblyAI se latência <1s for crítica.
...
**Recomendação CapIAu:** **YOLOv8** para detecção de objetos/ações, **Qwen2-VL-2B** para descrição semântica (cabe em 8GB VRAM), **InsightFace** para reconhecimento facial, **DeepFace** para análise de emoção.
...
**Recomendação CapIAu:** **SQLite** para metadados estruturados (takes, cenas, personagens, timecodes), **Qdrant** para busca semântica (embeddings de transcrições, descrições visuais, diálogos).
...
**Recomendação CapIAu:** **Qwen2.5-7B-Instruct (Q4_K_M)** como LLM principal — Apache 2.0 (sem restrições comerciais), excelente em PT-BR, cabe confortavelmente em 8GB VRAM com ~9 tokens/s. **Llama 3.2-3B** como fallback rápido para tarefas simples.
...
| Formato | Suporte NLE | Complexidade | Metadados | Recomendação CapIAu |
|:---|:---|:---|:---|:---|
...
| **Banco estruturado** | SQLite | PostgreSQL | Zero config, serverless, suficiente para escala do CapIAu; migração para PostgreSQL é trivial se necessário |
| **Busca semântica** | Qdrant (Docker) | Weaviate | Melhor performance self-hosted, menor footprint, filtering nativo, Apache 2.0 |
| **Grafo de relações** | SQLite (triplas) | Neo4j | Para a escala do CapIAu, triplas em SQLite são suficientes; evita infra adicional |
