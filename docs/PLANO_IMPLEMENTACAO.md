...
- [x] **E1.T7** Exportação nativa `.kdenlive` — **descartada (18/08/2026).**
  ❌ **Não será implementada, e o motivo é bom: o problema que ela resolveria deixou de existir.** O item nunca esteve neste plano; a promessa vivia em dois lugares da documentação — o README (anunciada como recurso PRONTO, texto que nunca correspondeu ao código) e a seção 3 do `docs/kdenlive_workflow.md` ("Visão de Futuro: Geração Direta de Arquivos .kdenlive"). Ambos corrigidos. **O Kdenlive lê OpenTimelineIO nativamente desde a versão 25.04**, e essa via cobre mais recursos que qualquer geração externa do formato. Testado em 18/08: o adaptador da comunidade `otio-kdenlive-adapter` (última versão `0.0.3`) **imprime aviso de descontinuação ao carregar**, recomendando explicitamente a via nativa; e **corrompe caminhos no Windows** — um clipe em `C:/Users/FGC/midia/clipeA.mp4` sai gravado como `/C:/Users/FGC/midia/clipeA.mp4`, com uma barra sobrando antes da letra do drive: caminho que o MLT não abre, deixando todas as mídias desvinculadas. O `otio-mlt-adapter` grava a URI `file:///` crua, com o mesmo efeito. Ambos foram desinstalados após o teste.
  ✅ **No lugar disso, consertado o que estava de fato quebrado na exportação:** o `<select id="select-export-format">` existia na barra de ferramentas mas **nenhum JavaScript o lia** — o botão chamava `exportTimelinePrompt()`, que abria um `prompt()` pedindo ao usuário para DIGITAR o formato e descartava silenciosamente a escolha do seletor. Agora `exportTimeline()` lê o seletor, e os rótulos deixam o destino explícito (`Kdenlive 25.04+ · OpenTimelineIO (.otio)` em primeiro, `Premiere / Resolve / Final Cut (.xml)`, `EDL · pista única (.edl)`).
  **Validado em 18/08 na timeline real "teste jlcut":** os 3 formatos exportam (`.otio` 11.757 bytes, `.xml` 7.066, `.edl` 714) e as 2 referências de mídia do `.otio` decodificam para caminhos existentes no `D:`. As URIs saem percent-encoded (`V%C3%ADdeos`, `%20`) — formato correto do padrão OTIO, decodificado na leitura, **não é corrupção**.
- [x] **E1.T8** Endereço da mídia no `.otio` incompatível com o Kdenlive — **corrigido (18/08/2026).**
  ❌ **Sintoma:** o `.otio` exportado não abria no Kdenlive 26.04.3. Duas mensagens: `Não é possível aceder ao ficheiro C:/Users/FGC/Downloads/timeline_6.otio` e, revelando a causa, `Cannot open file C:/Users/FGC/Downloads/file:///D:/makinof-monstro/V%C3%ADdeos/MONSTRO_2026-03-13/MONSTRO_20260313_192927_MVI_3132.MOV`.
  🔍 **Causa:** exportávamos `target_url` como URI (`Path(filepath).as_uri()` → `file:///D:/.../V%C3%ADdeos/...`). O padrão OpenTimelineIO admite isso, mas **o importador do Kdenlive não trata o campo como URI**: ele concatena a pasta do projeto na frente e não decodifica o percent-encoding. O resultado é o caminho impossível acima, e o projeto inteiro falha ao abrir. ⚠️ **Lição de método:** em 18/08 eu havia validado o `.otio` decodificando as URIs por conta própria e concluído "as mídias religam corretamente" — a validação confirmou que o ARQUIVO estava correto pelo padrão, não que o CONSUMIDOR o entendia. Só o teste no Kdenlive real mostrou o contrário.
  ✅ **Correção:** novo helper `_media_target_url(filepath, as_uri)` em `otio_export.py`. `generate_otio_timeline` ganhou o parâmetro `as_uri` (default `True`, retrocompatível) e `_export_in_process` passa `as_uri=(output_format != "otio")`. Ou seja: o **`.otio` grava caminho absoluto simples** (`D:/makinof-monstro/Vídeos/...`, via `as_posix()`), e o **`.xml` mantém a URI**, que é o que Premiere e Resolve esperam em `<pathurl>`. Conferido nos dois formatos após a mudança; o `.edl` não carrega caminho de mídia. 4 testes novos em `tests/test_f11_export.py`.
- [ ] **E1.T9 — Conciliar o banco desta máquina com o de casa** ⏳ **para 19/08/2026.**
  🔍 **Já confirmado em 18/08, antes da comparação:** ao abrir o `data/capiau.db` desta máquina, a migração precisou **criar** as colunas `title`, `category`, `category_confidence`, `burst_group_id`, `palette_temp`, `palette_hex`, `realm`, `role` e `status`. Como a migração só cria coluna ausente (`if "x" not in cols`, `src/db/schema.py`), **este banco é anterior a todo o trabalho de análise de 17–22/07** — prompts de visão com `max_tokens`, retry e fallback, triagem com few-shot, escala de plano, paleta, rajadas e universo de entidades.
  📊 **Estado medido aqui (18/08):** 541 vídeos (516 com descrição, **0 com categoria**), 1.424 fotos (1.424 com descrição, **0 com categoria**), **`media_segment` VAZIA** (0 segmentos — a segmentação real por shots/beats da Fase 16 nunca rodou neste banco), 0 rajadas, 0 correções de triagem, 45 entidades. Descrição média: 143 chars em vídeo, 163 em foto. Os **130 títulos** existentes vieram da rodada de "Gerar Títulos IA" da noite de 18/08, **não** da análise de casa.
  📌 **Para comparar com o E2.D1:** o backfill de 20/07 registrado neste plano classificou **7.042 pontos** e **5.618 segmentos** com `shot_scale`, e mediu o mix do acervo em 56% `processo` nos vídeos e 68% `cotidiano` nas fotos (967/1.424). Nada disso existe no banco daqui — é o marcador mais direto de qual dos dois tem a análise boa.
  🛠️ **Ferramenta:** `python scripts/comparar_bancos.py data/capiau.db <banco_de_casa.db>` — abre os dois em modo somente-leitura, mostra contagens lado a lado, aponta colunas e tabelas ausentes de cada lado e imprime amostras de descrição para julgar a qualidade a olho, não só pela contagem.
  ⚠️ **Antes de sobrescrever qualquer coisa:** o banco de casa provavelmente tem a análise boa, mas o daqui tem os 130 títulos executivos de 18/08 e as 45 entidades. Decidir o que preservar de cada lado — e tirar backup dos dois antes (há precedente no plano: `data/capiau.db.pre_fix_paths_*.bak`, em 14/07).
- [ ] **E1.T10 — Rerodar a geração de títulos com o código novo** ⏳ **para 19/08/2026. Provavelmente não há bug: só falta reiniciar.**
  🔍 **O que aconteceu (18/08, 20:45):** a geração travou em 24% (131/541), sem log de erro, no meio do item 131. Suspeita inicial: conflito de fila. **Não é.** O log do servidor mostra mensagens por item — `[LLM] [129/541] Analisando 'ARQUIVO.MOV'...` — e a string `"Analisando"` **não existe em nenhum lugar do código-fonte** (conferido com a árvore limpa, sem alterações pendentes).
  ⚠️ **Causa: módulo velho em memória.** O servidor (PID 16848) subiu às **20:29:11**; o `src/services/pipeline.py` foi salvo no disco às **20:33** — quatro minutos depois. O Python importa o módulo uma única vez, então o processo seguiu com a versão anterior de `regenerate_executive_titles`, que fazia **uma chamada de LLM por vídeo (541 chamadas)**. A versão commitada em `51d52f0` faz **uma chamada por micro-lote de 20 (28 chamadas)** e **nunca chegou a rodar**. É exatamente a armadilha já registrada no E1.T6 em 20/07: *"regra desta máquina: rodar SEM `--reload`; toda mudança de backend = reiniciar de verdade"*.
  ✅ **O que fazer:** reiniciar o servidor e disparar de novo. A versão nova tem **19× menos chamadas**, `timeout=40` e `try/except` por lote (um lote que falha é logado e o laço continua, em vez de morrer). Os 130 títulos já gravados são reaproveitáveis — a rota aceita `video_ids`, então dá para rodar só nos que faltam.
  📌 **Sobre a fila, para não ficar dúvida:** `regenerate_executive_titles` é lançada por `background_tasks.add_task`, ou seja roda no threadpool do FastAPI (~40 threads), **não** no `TASK_MANAGER.executor` de 2 workers usado pelas miniaturas. Ela ocupa uma thread por horas, mas não disputa com a fila de miniaturas. Não foi a causa do travamento.
  🔎 **Se travar de novo com o código novo**, aí sim investigar: (a) `is_cancelled` só é checado no início de cada micro-lote, então cancelar demora até 20 itens; (b) vale logar o `chunk_idx` antes da chamada, para saber em qual lote parou; (c) conferir se o `extract_json_from_markdown` engasga com resposta de 20 itens (payload bem maior que o de 1).
---
...
- [ ] **E2.D4 — Título executivo no índice** — Levar o `title` gerado pela IA (18/08, `regenerate_executive_titles`) para o payload do Qdrant e reindexar.
  📌 **Estado hoje:** os títulos executivos existem no banco e aparecem na interface (biblioteca, cartão do índice de rolagem, timeline), mas **não entram em nenhum índice vetorial**. Conferido em 18/08: o payload visual em `image_semantic.py` carrega `project_id`, `video_id`, `media_type`, `start_time`, `end_time`, `segment_id`, `shot_scale`, `category` e `camera_motion`; o de texto em `semantic.py` carrega `text`, `raw_text`, `tags` e `people`. Nenhum dos dois tem `title`.
  **O que falta:** (a) incluir `title` no payload das duas indexações; (b) decidir se o título também entra no TEXTO embedado (muda o vetor, exige reindexação completa) ou fica só como campo recuperável e filtrável (barato, sem reindexar); (c) sincronizar na hora quando o usuário renomeia pelo duplo clique, como já é feito com `category` em `sync_category_payload`; (d) backfill do acervo já titulado.
  **Por que importa:** um título como "Zé: Crítica ao primeiro corte" é a descrição mais densa e mais curada que existe da mídia — hoje ela é a única informação de qualidade que a busca não enxerga.
...
- [x] **P0.5** Teste: monkeypatch do client Qdrant levantando exceção (NUNCA subir 2ª instância real — lock).
*Aceite:* com o Qdrant tomado por outra instância, a busca mostra o aviso (não vazio) e o health aponta o problema.
✅ **(21/07) Validado — com 2 regressões encontradas na revisão e corrigidas na mesma sessão.** Os 4 testes de `tests/test_p0_qdrant_unavailable.py` passavam, mas não cobriam `/api/media/similar-batch`: o commit P0.2 tinha removido `results = []` de `similar_batch` (`src/api/routes/media.py`), quebrando o endpoint inteiro com `NameError` (HTTP 500) em toda chamada — provado via TestClient antes do fix. Também achado: o commit P0.1 tinha removido o `else` de `search_hybrid` (`src/services/rag.py`) que gerava a explicação "Correspondência conceitual (Semântica)" para resultados sem `match_source` — degradava com graça (só sumia o tooltip) mas afetava o tipo de resultado mais comum. Ambos corrigidos; 4 testes de regressão novos cobrindo os dois casos + o caminho feliz do `similar-batch`. Endurecimentos adicionais: (a) os 4 endpoints do P0 agora distinguem `QdrantUnavailableError` (índice fora) de qualquer outra `Exception` (`index_status="error"`, mensagem real do erro, sem afirmar "lock do Qdrant" para bugs de código — o próprio NameError acima teria se disfarçado de índice indisponível antes deste ajuste); (b) `check_health()` agora tenta reconectar sozinho (`_try_init()`) antes de reportar falha — o cenário da porta 8001 (instância zumbi derrubada) se cura sem restart manual, coberto por teste com `QdrantClient` mockado; `ImageSearch` parou de guardar seu próprio `client` e passou a lê-lo sempre do singleton via `@property`, para acompanhar a reconexão. **Nota de ambiente:** rodar a suíte com o servidor real aberto na mesma máquina gera um log de "Storage folder already accessed" (o singleton de teste tenta abrir o mesmo `data/qdrant.db`) — inofensivo pois nenhum teste depende da conexão real (tudo mockado), mas confirma na prática o motivo do aviso P0 em si.
- [x] **P0.6 (novo, 21/07)** O chat (RAG legado e Agente de Edição) ainda degradava em silêncio: `search_hybrid` já tratava `QdrantUnavailableError` internamente, mas nenhum dos dois caminhos de chat lia `index_status`/`warning` — a IA respondia normalmente sem avisar que estava sem contexto do acervo.
  ✅ **Fechado.** `RAGService.chat` e `ChatAgentService.chat_with_agent` passaram a chamar `search_hybrid(..., return_meta=True)`; quando o índice está fora, uma nota é injetada no `system_prompt` (o modelo avisa o usuário em linguagem natural) e `index_status`/`warning` voltam em todo retorno (inclusive nos early-returns de erro). A tool `search_media` do agente também passou a devolver "índice indisponível" no lugar de lista vazia. UI: `chat.js` anexa o aviso (mesmo componente visual do banner de busca) dentro da bolha do assistente quando `response.index_status !== "ok"` — validado ao vivo injetando um `indexWarning` sintético em `STATE.chatHistory` no servidor real rodando (sem subir 2ª instância nem derrubar o Qdrant de verdade). 5 testes novos em `tests/test_p0b_chat_qdrant.py` (Qdrant e OpenRouter mockados, padrão do P0.5); suíte completa (109/110 — a 1 falha é o `test_f0_otio_export` pré-existente, só roda dentro do venv312) verde.
...
- [x] **P1.1** Migração: `byte_hash` (sha256 dos bytes) e `content_hash` (sha256 do texto extraído normalizado: minúsculas + espaços colapsados) em `production_doc`.
- [x] **P1.2** Upload: `byte_hash` idêntico → 409 "documento idêntico já existe"; `content_hash` idêntico → 409 "mesmo conteúdo em outro formato (id X)"; similaridade de texto > ~0.9 (difflib/embedding) → resposta "possível nova versão de X" com opção `replace_doc_id` que **apaga os chunks antigos do Qdrant** e o doc antigo antes de indexar o novo.
- [x] **P1.3** UI: diálogo no upload com as 3 situações (idêntico/mesmo conteúdo/nova versão).
...
✅ **(21/07) P1.1–P1.3 implementados.** `ProjectRepository.hash_doc_bytes`/`hash_doc_content` (sha256 completo, sem truncar — diferente do `compute_hash` parcial de mídia grande em `ingest.py`); `upload_document` (`src/api/routes/projects.py`) checa byte_hash → content_hash → similaridade (`difflib.SequenceMatcher.quick_ratio`, limiar novo `docs.version_similarity_threshold` no `SETTINGS_REGISTRY`, default 0.90) antes do INSERT; `replace_doc_id` reusa `delete_production_doc_vectors` + `delete_document` já existentes do endpoint DELETE. UI (`library.js`) distingue só o caso `near_version` com `confirm()` + reenvio; os outros 409 caem no alerta genérico (mensagem já vem pronta do backend) — sem infra de modal nova, mesmo idioma alert/confirm do resto do app. 6 testes novos em `tests/test_p1_doc_dedupe.py` (banco temporário, Qdrant sempre mockado). **Nota de ambiente descoberta ao testar:** `src/api/server.py` chama `init_db()` no import do módulo (linha 31) — todo teste que importa `app` migra o banco **real** na hora (aditivo, mesmo padrão `PRAGMA table_info` de sempre; já era assim antes desta sessão nos testes do P0). Migração já rodou contra `data/capiau.db` real durante os testes desta sessão; conferido read-only depois: `content_hash` populado nos 2 docs existentes (id 3 e 8, hashes diferentes entre si — o PDF perdeu texto na extração, por isso `same_content` não pega esse par, só o `near_version` pegaria), `byte_hash` NULL nos dois (esperado, são linhas legadas). **P1.4 pendente:** apagar o `.pdf` id 8 pelo DELETE já existente — combinar com o usuário o momento (servidor estava aberto durante esta sessão).

### P2 — E3.C1 antecipado: extração estruturada do roteiro + bloco de contexto compacto ✅ implementado em 21/07/2026
...
- [x] **P2.1** Prompt `script_extract` no PROMPT_REGISTRY (com `label`/`category`/`variables`): entrada = chunk do roteiro; saída JSON = personagens (nome + 1 linha), cenas (número, heading, sinopse de 1 frase, personagens, props, locação), objetos-chave. Chunking com sobreposição de 1 cena; fusão dos resultados por número de cena.
  ✅ **(21/07) Redesenhado antes de codar: o plano original usava um regex único de sluglines para segmentar cenas** — testado contra variações normais de formatação, falhava em 7 de 10 casos (inclusive `.fdx`, o formato com a MELHOR estrutura disponível) e o pior, falhava **em silêncio**: um draft numerado caía de 111 para 7 cenas sem disparar nenhum alarme. Substituído por `src/services/script_format.py`: cascata de 5 camadas — (0) estrutura nativa do formato (`.fdx`, `.fountain`), (1) biblioteca de padrões candidatos competindo por pontuação, (2) **validação do resultado** (a peça que faltava: discriminador é a MEDIANA do tamanho de cena — 610 chars segmentado certo vs. 10.810 quebrado; a dispersão sozinha NÃO separa os dois casos, achado só na calibração), (3) o LLM identifica a convenção quando as heurísticas falham (1 chamada barata, mesma validação), (4) modo `prose` para documento sem estrutura de cena (não inventa cenas). Numeração de cena sempre por POSIÇÃO no documento (nunca pelo LLM) — resolve headings repetidos e roteiros sem numeração no texto. `GET /api/docs/{id}/structure-preview` roda as camadas grátis antes de qualquer chamada, para o usuário confirmar. Chunking/fusão em `src/services/script_extract.py` (`build_chunks`, `split_chunk` para recuperar de truncamento — retry sozinho não resolve, precisa reduzir a saída esperada —, `merge_chunk_results`). 28+20 testes; validado no roteiro real: 111 cenas, 6 chunks, confiança 1.0.
- [x] **P2.2** Tabela `scene(project_id, doc_id, number, heading, synopsis, characters_json, props_json, location, created_at)` (migração segura) + entidades sugeridas em `entity` com `status='suggested'` (coluna nova; só `confirmed` entra em prompts — regra do E3.C3 original).
  ✅ **(21/07)** + tabela `script_extraction` (cache + custo). Migração aplicada no banco real: as 45 entidades existentes viraram `confirmed` (comportamento anterior preservado). `upsert_suggested_entity` nunca rebaixa confirmada nem ressuscita rejeitada (lápide contra re-sugestão). `replace_scenes_for_doc` preserva curadoria humana por número entre reextrações (achado na revisão — o desenho original perdia confirmações no `force`). 14 testes.
- [x] **P2.3** Cache: extração roda 1x por `content_hash`; re-upload da mesma versão reusa.
  ✅ **(21/07)** `run_script_extraction` em `src/services/script_extract.py`: cache checado antes de qualquer chamada; falha persistente de chunk (após retries + split) aborta a rodada inteira sem gravar nada parcial; guarda 409 contra duas rodadas simultâneas do mesmo doc. 16 testes; validado ponta a ponta no roteiro real (cópia descartável do banco): 6 chamadas, 111 cenas persistidas.
- [x] **P2.4** `script_context_block` compilado (~1,5–2k tokens: logline + personagens confirmados + locações + props) injetável em `triage`/`vision` via setting `context.script_block_enabled` (**default false** até validar em ~10 reanálises A/B — convenção anti-viés: o bloco descreve o UNIVERSO do filme, nunca afirma que o material É do filme).
  ✅ **(21/07)** `_script_context_block()` em `prompt_templates.py`, molde do `_triage_feedback_block`. Injeção em `get_triage_prompt` e `get_vision_prompt` — **não** em `get_photo_vision_prompt`: essa função monta sua base chamando `get_vision_prompt` internamente, então injetar nos dois duplicaria o bloco no prompt de foto (divergência do plano original só visível lendo o código real; corrigida antes de implementar). 16 testes; confirmado no projeto real com o gate desligado (default): bloco vazio, prompt idêntico a antes.
- [x] **P2.5** UI mínima de curadoria (E3.C3 do plano): lista de personagens/cenas extraídos com aceitar/rejeitar em massa (aba Docs → botão "Extrair estrutura").
  ✅ **(21/07)** Botão "Extrair estrutura" no card de doc `script` (aba Docs) → preview sem custo com estratégia/contagem/amostra de cenas num `confirm()` (oferece "analisar com IA" se `needs_review`) → dispara a extração → badge no card vira clicável ("N cenas extraídas — ver/curar") → seção de curadoria com checkboxes (personagens/locações/objetos sugeridos e cenas) e aceitar/rejeitar em massa. Validado no navegador contra o app real: XSS neutralizado no `esc()`, "aceitar todos" ignora o estado do checkbox corretamente, chamadas de API disparadas com os IDs certos.
*Aceite:* roteiro real extraído com cenas/personagens conferíveis pelo usuário; custo de extração logado; reanálise de 5 itens com o bloco ligado mostra nomes/termos do filme nas descrições sem inventar vínculo. **Backend provado ponta a ponta contra o roteiro real; a conferência visual final (UI) é a parada de teste do usuário abaixo.**
