# CapIAu-Talho: APIs Agregadoras — O "Coração" da Estratégia Híbrida

**Data:** 02/06/2026  
**Versão:** 2.1  
**Escopo:** Análise aprofundada de APIs agregadoras (AI Gateways / LLM Routers) como camada central de orquestração entre o cluster GPU local, APIs pagas individuais, e o motor do CapIAu-Talho. Inclui comparativo completo dos principais players, arquitetura recomendada, custos, e estratégia de routing inteligente.

---

## TL;DR — Por que uma API Agregadora muda tudo

Uma **API agregadora** (também chamada de *AI Gateway* ou *LLM Router*) é uma camada de software que se posiciona **entre o CapIAu-Talho e todos os provedores de IA** — modelos locais no cluster, APIs de LLM, APIs de visão, APIs de transcrição, etc. Em vez de o CapIAu-Talho integrar diretamente com dezenas de provedores (OpenAI, Anthropic, Groq, DeepSeek, AWS, Azure, Google, etc.), ele se conecta **uma única vez** à agregadora, e ela cuida do resto. Isso reduz a complexidade de integração de **N conexões para 1**, habilita **routing inteligente** (enviar tarefas simples para modelos baratos, complexas para modelos caros), provê **fallback automático** (se um provedor cai, troca para outro instantaneamente), e oferece **observabilidade unificada** (custo, latência, uso — tudo em um dashboard). A escolha recomendada para o CapIAu-Talho é **LiteLLM (self-hosted)** para controle total e custo zero de gateway, com **Cloudflare AI Gateway** como backup managed se necessário.

---

## 1. O Que é uma API Agregadora e Por que o CapIAu-Talho Precisa de uma

### 1.1 O Problema: Integração Direta é um Pesadelo

Sem uma agregadora, o CapIAu-Talho precisaria integrar diretamente com cada provedor:
...
COM AGREGADORA (LiteLLM/OpenRouter — simples):
  ...
  CapIAu-Talho ──→ API Agregadora ──→ OpenAI
                          ──→ Anthropic
                          ──→ Groq
                          ──→ DeepSeek
                          ──→ AWS
...
### 1.3 Benefícios Específicos para o CapIAu-Talho

| Benefício | Descrição | Impacto no CapIAu-Talho |
|:---|:---|:---|
...
| **Formato Unificado** | Todas as respostas vêm no formato OpenAI (independente do provedor) | Código do CapIAu-Talho não muda quando troca de modelo |
...
#### **LiteLLM — A Escolha do CapIAu-Talho**

**LiteLLM** é um proxy server open-source (MIT license) que expõe uma única API OpenAI-compatible e roteia para 100+ provedores. É a escolha recomendada para o CapIAu-Talho por ser **self-hosted, gratuito, e extremamente flexível**.
...
**Como funciona no CapIAu-Talho:**

```python
# O CapIAu-Talho faz UMA chamada — LiteLLM decide para onde enviar
import openai
...
**Veredito:** Bom para prototipagem rápida, mas **LiteLLM é superior para produção** do CapIAu-Talho por ser gratuito e self-hosted.

#### **Cloudflare AI Gateway — Opção Edge**

Se o CapIAu-Talho já usa Cloudflare (CDN, DNS, Workers), o AI Gateway é uma extensão natural. Oferece **350+ modelos**, caching edge, rate limiting, e DLP (Data Loss Prevention). O **markup é de 5%** em unified billing.
...
| RBAC e team management | Overkill para CapIAu-Talho no estágio atual |
| Guardrails avançados | |

**Veredito:** Considerar apenas quando o CapIAu-Talho atingir **escala enterprise** com requisitos de compliance rigorosos.

---

## 3. Arquitetura do CapIAu-Talho com API Agregadora
...
│                           │                                                            │
│                    ┌──────┴──────┐                                                      │
│                    │  FastAPI    │  ← API REST do CapIAu-Talho (única integração)            │
│                    └──────┬──────┘                                                      │
└───────────────────────────┼─────────────────────────────────────────────────────────────┘
                            │
                            ▼
...
```
1. Editor no CapIAu-Talho clica "Analisar cena #12"
   │
...
O CapIAu-Talho precisa de um classificador que decide qual "nível" de modelo usar para cada tarefa:

```python
# Módulo de routing no CapIAu-Talho (camada antes de chamar LiteLLM)
class TaskRouter:
...
| Nível | % das Tarefas | Modelos | Custo/1M tokens | Exemplos no CapIAu-Talho |
|:---|:---|:---|:---|:---|
...
| **PostgreSQL** | $0 (compartilhado) | Pode usar o mesmo SQLite/PostgreSQL do CapIAu-Talho |
...
## 6. Recomendação Final para o CapIAu-Talho
...
| **Fase 3** | 2 dias | Implementar classificador de complexidade no CapIAu-Talho |
...
```yaml
# litellm_config.yaml — Configuração completa para o CapIAu-Talho
...
*Documento complementar ao plano expandido do CapIAu-Talho. Para o contexto completo (cluster GPU, APIs individuais, Asset Sourcing, cenários de custo), consulte `capiau_apis_cluster_expandido.md`.*
