...
SETTINGS_REGISTRY: List[Dict[str, Any]] = [
    ...
    {
        "key": "script.min_coverage", "type": "float", "default": 0.6, "min": 0.1, "max": 1.0, "step": 0.05,
        "label": "Roteiro: cobertura mínima do texto",
        "help": "Que fração do roteiro precisa ficar dentro de alguma cena para a detecção ser aceita. Serve de alarme: se o programa só reconheceu cabeçalhos numa parte do arquivo, é sinal de que o formato não foi entendido.",
        "help_tech": "validate_segmentation(): soma dos tamanhos de cena / len(content).",
        "category": "themes_search", "level": "pro", "scope": "both", "requires_reprocess": True,
    },
    {
        "key": "script.max_median_chars", "type": "int", "default": 4000, "min": 800, "max": 20000, "step": 100,
        "label": "Roteiro: tamanho máximo típico de uma cena",
        "help": "Se as 'cenas' detectadas forem grandes demais, é porque o programa não reconheceu o padrão de cabeçalho e juntou várias cenas num bloco só. No roteiro deste projeto a cena mediana tem ~610 caracteres.",
        "help_tech": "validate_segmentation(): mediana dos tamanhos de segmento. Discriminador principal (medido: 610 chars segmentado certo vs. 10.810 chars quando o padrão falha). A dispersão max/mediana NÃO separa os dois casos.",
        "category": "themes_search", "level": "pro", "scope": "both", "requires_reprocess": True,
    },
    {
        "key": "script.max_scene_ratio", "type": "float", "default": 25.0, "min": 3.0, "max": 100.0, "step": 1.0,
        "label": "Roteiro: desigualdade máxima entre cenas",
        "help": "Rede secundária: pega o caso de um trecho inteiro do roteiro cujos cabeçalhos não foram reconhecidos, o que produz uma cena gigante no meio de cenas normais.",
        "help_tech": "validate_segmentation(): max_len/median_len. Limite folgado de propósito — o roteiro real saudável já dá 9,3.",
        "category": "themes_search", "level": "pro", "scope": "both", "requires_reprocess": True,
    },
    {
        "key": "script.llm_format_detection", "type": "bool", "default": True,
        "label": "Roteiro: pedir ajuda à IA em formato desconhecido",
        "help": "Quando nenhum padrão conhecido funciona, envia um trecho do documento para a IA identificar como aquele roteiro marca as cenas. Custa uma chamada barata, uma única vez por documento.",
        "help_tech": "Camada 3 de detect_structure(): prompt 'script_format_detect' devolve uma regex, que passa pela mesma validação das camadas anteriores.",
        "category": "themes_search", "level": "pro", "scope": "both", "requires_reprocess": False,
    },
    {
        "key": "script_extract.chunk_chars", "type": "int", "default": 24000, "min": 8000, "max": 60000, "step": 1000,
        "label": "Roteiro: tamanho do trecho enviado à IA",
        "help": "Quantos caracteres do roteiro entram em cada chamada de extração. Menor = mais chamadas (mais lento, mais caro) mas com menos risco de a resposta da IA ser cortada. No roteiro deste projeto (~130k chars), o padrão gera 6 trechos.",
        "help_tech": "build_chunks() em script_extract.py — fronteira sempre em limite de cena, nunca corta uma cena ao meio.",
        "category": "themes_search", "level": "pro", "scope": "both", "requires_reprocess": False,
    },
    {
        "key": "script_extract.max_tokens", "type": "int", "default": 4000, "min": 1000, "max": 8000, "step": 500,
        "label": "Roteiro: teto de tokens de saída por trecho",
        "help": "Teto de tamanho da resposta da IA por trecho extraído. Se a extração falhar por corte, o sistema tenta dividir o trecho em dois automaticamente antes de desistir.",
        "help_tech": "max_tokens do payload OpenRouter em call_text_llm(); truncamento aciona split_chunk() no executor.",
        "category": "themes_search", "level": "pro", "scope": "both", "requires_reprocess": False,
    },
    {
        "key": "script_extract.temperature", "type": "float", "default": 0.1, "min": 0.0, "max": 1.0, "step": 0.05,
        "label": "Roteiro: criatividade da extração",
        "help": "Mantenha baixo: extração estruturada deve ser fiel ao texto, não criativa.",
        "help_tech": "temperature do payload OpenRouter em call_text_llm().",
        "category": "themes_search", "level": "pro", "scope": "both", "requires_reprocess": False,
    },
    {
        "key": "script_extract.timeout", "type": "int", "default": 120, "min": 30, "max": 300, "step": 10,
        "label": "Roteiro: tempo limite por chamada (s)",
        "help": "Quanto tempo esperar a resposta da IA antes de considerar falha e tentar de novo.",
        "help_tech": "timeout do requests.post em call_text_llm().",
        "category": "themes_search", "level": "pro", "scope": "both", "requires_reprocess": False,
    },
    {
        "key": "script_extract.max_retries", "type": "int", "default": 2, "min": 1, "max": 5, "step": 1,
        "label": "Roteiro: tentativas por trecho",
        "help": "Quantas vezes tentar o modelo principal antes de cair no modelo de reserva.",
        "help_tech": "retries de call_text_llm() no modelo llm.text_model antes do fallback llm.text_model_fallback.",
        "category": "themes_search", "level": "pro", "scope": "both", "requires_reprocess": False,
    },
    {
