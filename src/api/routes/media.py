...
from src.api.schemas import ExternalPathIngest, LabelFacePayload
...
def list_videos(project_id: int = Query(1), conn: sqlite3.Connection = Depends(get_db_conn)):
    ...
    return MediaRepository.list_videos(conn, project_id)
...
def label_face(face_id: int, payload: LabelFacePayload, conn: sqlite3.Connection = Depends(get_db_conn)):
    """Rotula um rosto específico identificado com o nome da pessoa."""
    try:
        MediaRepository.label_face(conn, face_id, payload.name)
        conn.commit()
